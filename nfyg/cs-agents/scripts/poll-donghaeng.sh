...
STATE_FILE="$(dirname "$0")/../state/donghaeng-inbox.md"
LAST_SEEN=""

echo "=============================="
echo " 동행 채널톡 CS 문의 모니터"
echo " 30초마다 갱신"
echo "=============================="
echo ""

if [[ "$KEY" == *"DONGHAENG"* ]]; then
  echo "⚠️  동행 채널톡 API 키를 설정해주세요."
  echo "   scripts/poll-donghaeng.sh 의 KEY / SECRET 을 채워넣으세요."
  read
fi
...
" 2>/dev/null > "$STATE_FILE"
    ...
    echo "  → state/donghaeng-inbox.md 업데이트됨"
    echo ""
  fi
